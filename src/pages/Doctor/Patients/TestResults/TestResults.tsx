...
const TestResults: React.FC = () => {
  ...
  const determineResultStatus = (results: string): 'Normal' | 'Abnormal' | 'Pending' => {
    const lowerResults = results.toLowerCase();
    if (lowerResults.includes('bình thường') || lowerResults.includes('normal') || lowerResults.includes('âm tính')) {
      return 'Normal';
    } else if (lowerResults.includes('bất thường') || lowerResults.includes('abnormal') || lowerResults.includes('dương tính')) {
      return 'Abnormal';
    }
    return 'Pending';
  };
        ...
        <div className="header">
          <div className="header-icon">
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L13.09 8.26L19 7L17.26 13.09L23 12L21.74 18.26L18 17L19.26 23L13 21.74L12 18L11 21.74L4.74 23L6 17L2 18.26L3.26 12L9 13.09L7 7L12.91 8.26L12 2Z" fill="#00B4C6"/>
            </svg>
          </div>
          ...
          <div className="result-box">
            <div className="result-header">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" stroke="#00B4C6" strokeWidth="2"/>
                <polyline points="22,4 12,14.01 9,11.01" stroke="#00B4C6" strokeWidth="2"/>
              </svg>
            ...
            <div className="result-content">
              <div className="result-item">
                ...
                <span className="result-value">{submittedResult.patientName}</span>
              </div>
              
              <div className="result-item">
                <span className="result-label">Mã bệnh nhân:</span>
                <span className="result-value">{submittedResult.patientId}</span>
              </div>
              
              <div className="result-item">
                <span className="result-label">Loại xét nghiệm:</span>
                <span className="result-value">{getTestTypeLabel(submittedResult.testType)}</span>
              </div>
              
              <div className="result-item">
                <span className="result-label">Ngày xét nghiệm:</span>
                <span className="result-value">
                  {new Date(submittedResult.testDate).toLocaleDateString('vi-VN')}
                </span>
              </div>
              
              <div className="result-item">
                <span className="result-label">Trạng thái:</span>
                <span className={`result-status ${getStatusColor(submittedResult.resultStatus)}`}>
