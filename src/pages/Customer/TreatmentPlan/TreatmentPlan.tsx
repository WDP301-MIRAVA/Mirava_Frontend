...
import { TreatmentPlanService, type TreatmentPlan as ApiTreatmentPlan } from '../../../services/treatmentPlan.service';
...
const TreatmentPlan: React.FC = () => {
  ...
  const transformApiDataToTreatmentEvents = (apiData: ApiTreatmentPlan[]): TreatmentEvent[] => {
    ...
    apiData.forEach((plan) => {
      ...
      if (plan.ovarianStimulation) {
        ...
        stimStartDate.setDate(stimStartDate.getDate() + plan.ovarianStimulation.startDay - 1);
        ...
        plan.ovarianStimulation.monitoringSchedule.forEach((monitoring) => {
          ...
          if (monitoring.type.toLowerCase().includes('ultrasound') || monitoring.type.toLowerCase().includes('siêu âm')) {
          ...
          } else if (monitoring.type.toLowerCase().includes('blood') || monitoring.type.toLowerCase().includes('hormone')) {
            eventType = 'test';
      ...
      if (plan.hcgInjection && plan.hcgInjection.plannedDate) {
        ...
        events.push({
          ...
          details: 'Tiêm thuốc HCG để kích thích trưởng thành cuối cùng của trứng',
      ...
      if (plan.eggRetrieval && plan.eggRetrieval.plannedDate) {
        ...
        events.push({
          ...
          instructions: plan.eggRetrieval.notes || 'Nhịn ăn uống từ 22:00 ngày hôm trước. Đến bệnh viện lúc 07:00',
          time: '09:00'
        });
      }
      ...
      if (plan.embryoTransfer && plan.embryoTransfer.plannedDate) {
        ...
        events.push({
          ...
          instructions: 'Uống đủ nước, không cần nhịn ăn. Nghỉ ngơi sau thủ thuật',
          time: '10:30'
        });
      }
      ...
      if (plan.postTransferMonitoring) {
        ...
        if (plan.postTransferMonitoring.ultrasoundCheckDate) {
          const ultrasoundDate = new Date(plan.postTransferMonitoring.ultrasoundCheckDate);
          events.push({
  ...
  useEffect(() => {
    const fetchTreatmentPlan = async () => {
      try {
        ...
        if (!patientId && typeof window !== 'undefined' && window.sessionStorage) {
        ...
        const response = await TreatmentPlanService.getTreatmentPlanByPatientId(patientId);
        
        console.log('API Response:', response);
        
        if (response.data && response.data.success && response.data.data && response.data.data.length > 0) {
          const transformedEvents = transformApiDataToTreatmentEvents(response.data.data);
  ...
  const selectedDateEvents = selectedDate ? treatmentPlan.filter(event => event.date === selectedDate) : [];
      ...
      {activeView === 'list' ? (
        <div className="list-view">
          <div className="timeline">
            {treatmentPlan.map((event) => (
              <div key={event.id} className={`timeline-item ${getEventTypeClass(event.type)}`}>
                ...
                <div className="timeline-content">
                  ...
                  {event.medication && (
                    <div className="medication-info">
                      ...
                      {event.dosage && <span> - Liều lượng: {event.dosage}</span>}
                    </div>
                  )}
                  {event.instructions && (
        ...
        <div className="calendar-view">
          <div className="calendar-header">
            ...
            <h2>{currentMonth.toLocaleDateString('vi-VN', { month: 'long', year: 'numeric' })}</h2>
            ...
            <div className="calendar-days">
              {getDaysInMonth(currentMonth).map((day, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`calendar-day ${day ? 'active' : 'inactive'} ${events.length > 0 ? 'has-events' : ''}`}
                      ...
                      <>
                        ...
                        {events.length > 0 && (
                          <div className="event-indicators">
                            {events.map(event => (
                              <div key={event.id} className={`event-dot ${getEventTypeClass(event.type)}`}></div>
                            ))}
                          </div>
                        )}
                      </>
