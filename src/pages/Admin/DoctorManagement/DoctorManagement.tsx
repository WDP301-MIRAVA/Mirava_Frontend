...
import { Search, Plus, Edit, Trash2, Ban, Check, Star, User } from 'lucide-react';
...
const DoctorManagement: React.FC = () => {
  const [doctors, setDoctors] = useState<Doctor[]>([
    ...
    },
    {
      ...
      status: 'active',
      imageUrl: 'https://images.unsplash.com/photo-1594824797298-a3c096c78c38?w=100&h=100&fit=crop&crop=face',
    ...
    },
    {
      ...
      description: 'Bác sĩ chuyên khoa chấn thương chỉnh hình, phẫu thuật xương khớp',
      rating: 4.6,
  ...
  const [sortBy, setSortBy] = useState<'name' | 'specialty' | 'createdDate'>('name');
  ...
  const showConfirmDialog = (type: 'delete' | 'block' | 'activate', doctorId: string, doctorName: string) => {
      ...
      <div className="controls">
        ...
        <div className="sort-container">
          ...
          >
            ...
            <option value="createdDate-desc">Sắp xếp: Ngày tạo (Mới nhất)</option>
          </select>
        </div>
      </div>
                ...
                <td>
                  <div className="action-buttons">
                    ...
                    <button 
                      ...
                      title="Xóa"
                      onClick={() => showConfirmDialog('delete', doctor.id, doctor.userName)}
                    >
                      <Trash2 size={16} />
                    </button>
                    <button 
                      className={`action-btn ${doctor.status === 'active' ? 'block-btn' : 'activate-btn'}`}
                      title={doctor.status === 'active' ? 'Khóa tài khoản' : 'Kích hoạt tài khoản'}
                    ...
                    >
                      {doctor.status === 'active' ? <Ban size={16} /> : <Check size={16} />}
                    </button>
                  </div>
                </td>
              </tr>
          ...
          <div className="modal confirmation-modal">
            ...
            <div className="modal-body">
              <p>
                ...
                {confirmAction.type === 'activate' && 'kích hoạt'}
                {' '} bác sĩ <strong>{confirmAction.doctorName}</strong>?
              </p>
            </div>
            <div className="modal-footer">
        ...
        <div className="modal-overlay">
          <div className="modal doctor-detail-modal">
            <div className="modal-header">
              ...
              <button onClick={() => setShowDoctorDetail(false)} className="close-btn">&times;</button>
            </div>
            <div className="modal-body">
              <div className="doctor-profile">
                <div className="doctor-avatar-large">
                  {selectedDoctor.imageUrl ? (
                    <img src={selectedDoctor.imageUrl} alt={selectedDoctor.userName} />
                  ) : (
                    <User size={80} />
                  )}
                </div>
                  ...
                  <div className="rating">
                    {Array.from({length: 5}, (_, i) => (
                      <Star 
                        key={i} 
                        size={16} 
                        className={i < Math.floor(selectedDoctor.rating) ? 'star-filled' : 'star-empty'}
                      />
                    ))}
                    <span className="rating-text">({selectedDoctor.rating}/5.0)</span>
                  </div>
                  <div className="info-grid">
                    ...
                    <div className="info-item">
                      ...
                      <span className={`status-badge ${selectedDoctor.status}`}>
                        {selectedDoctor.status === 'active' ? 'Hoạt động' : 'Bị khóa'}
                      </span>
                    </div>
                  </div>
            ...
            <div className="modal-footer">
              <button onClick={() => setShowDoctorDetail(false)} className="cancel-btn">
                Đóng
              </button>
            </div>
          </div>
