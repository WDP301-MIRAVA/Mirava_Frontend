...
const FeedbackManagement: React.FC = () => {
  ...
  const [doctorCache, setDoctorCache] = useState<Map<string, Doctor>>(
    new Map()
  );
  const [userCache, setUserCache] = useState<Map<string, User>>(new Map());
  const [serviceCache, setServiceCache] = useState<Map<string, ServiceData>>(
    new Map()
  );
  ...
  const fetchFeedbacks = async (page: number = 1) => {
    ...
    try {
          ...
          response.feedbacks.map(async (feedback: FeedbackResponse) => {
            ...
            if (feedback.doctor && feedback.doctor._id) {
              ...
              if (doctorDetails) {
                ...
                doctorSpecialty =
                  doctorDetails.specialty || feedback.doctor.specialty || "";
              } else {
            ...
            if (feedback.service) {
              if (
                typeof feedback.service === "object" &&
      ...
      if (err instanceof FeedbackApiError) {
        if (err.status === 401) {
          setError(
            "Phiên đăng nhập đã hết hạn hoặc bạn không có quyền admin. Vui lòng đăng nhập lại."
          );
      ...
      <div className="feedback-pagination">
        ...
        {startPage > 1 && (
          <>
            ...
            {startPage > 2 && (
              <span className="feedback-pagination-dots">...</span>
            )}
          </>
        )}
        {pages}
    ...
    return (
      <div className="feedback-management-container">
        <div className="feedback-management-header">
          <h1 className="feedback-management-title">
    ...
    <div className="feedback-management-container">
      ...
      {error && feedbacks.length > 0 && (
        <div className="feedback-error-banner">
          ...
          <button
            className="feedback-error-close"
          ...
          <tbody>
            {filteredAndSortedFeedbacks.map((feedback) => (
              <tr key={feedback.id} className="feedback-table-row">
                <td className="feedback-doctor-name" data-label="Bác sĩ">
                  {feedback.doctorName}
                </td>
                <td className="feedback-doctor-specialty" data-label="dịch vụ">
                  {feedback.doctorSpecialty || "N/A"}
                </td>
                <td className="feedback-patient-info" data-label="Bệnh nhân">
                  <div className="feedback-patient-name">
                    {feedback.patientName}
                  </div>
                  {feedback.patientEmail && (
                    <div className="feedback-patient-email">
                      {feedback.patientEmail}
                    </div>
                  )}
                </td>
                <td className="feedback-service-name" data-label="Dịch vụ">
                ...
                <td className="feedback-date" data-label="Ngày">
      ...
      {deleteConfirm && (
        <div className="feedback-modal-overlay">
          <div className="feedback-confirm-modal">
            <h3>Xác nhận xóa</h3>
            <p>
              Bạn có chắc chắn muốn xóa phản hồi này? Hành động này không thể
              hoàn tác.
            </p>
            <div className="feedback-modal-actions">
              <button
